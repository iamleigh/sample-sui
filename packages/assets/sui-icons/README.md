...
>Place the `suicons` class with `&lt;span&gt;` tag in your HTML file, like this. Then, use the <strong>icon name</strong> along with the `suicons-` class prefix. Don't forget to hide it for screen readers.

```html
<span class="suicons suicons--plugin-smartcrawl" aria-hidden="true"></span>
```
